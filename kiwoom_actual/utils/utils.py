...
from config.config import api_key, api_secret_key, host
...
class KiwoomTR:
    ...
    def fn_kt10000(self, data, cont_yn='N', next_key=''):
        ...
        try:
            ...
            # API 응답에서 주문번호 확인 (여러 가능한 키 확인)
            order_no = None
            possible_keys = ['ord_no', 'order_no', 'orderNo', 'ordNo']

            for key in possible_keys:
                if key in response_data:
                    order_no = response_data[key]
                    break

            if order_no is None:
                # 전체 응답 로그 출력
                log_error(f"매수주문 응답에서 주문번호를 찾을 수 없음. 응답 데이터: {response_data}")
                # 기본값으로 빈 문자열 또는 타임스탬프 사용
                import time
                order_no = f"UNKNOWN_{int(time.time())}"
            ...
            log_error(f"매수주문 예외 - 종목: {종목코드}, 에러: {str(e)}")
            raise
    ...
    def fn_kt10001(self, data, cont_yn='N', next_key=''):
        ...
        try:
            ...
            if order_no is None:
                log_error(f"매도주문 응답에서 주문번호를 찾을 수 없음. 응답 데이터: {response_data}")
                import time
                order_no = f"UNKNOWN_{int(time.time())}"
            ...
            log_error(f"매도주문 예외 - 종목: {종목코드}, 에러: {str(e)}")
            raise
    ...
    def fn_kt10002(self, data, cont_yn='N', next_key=''):
        ...
        try:
            ...
            if order_no is None:
                log_error(f"정정주문 응답에서 주문번호를 찾을 수 없음. 응답 데이터: {response_data}")
                import time
                order_no = f"UNKNOWN_{int(time.time())}"
            ...
            log_error(f"정정주문 예외 - 종목: {종목코드}, 에러: {str(e)}")
            raise
